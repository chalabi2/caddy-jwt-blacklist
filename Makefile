...
xcaddy-test: ## Build and run tests with real Redis
	@echo "Testing with Redis integration..."
	@echo "Ensure Redis is running on localhost:6379 for integration tests"
	$(GOTEST) -v ./... -tags=integration

## Redis Setup
redis-start: ## Start Redis for testing (requires Docker)
	docker run -d --name redis-jwt-test -p 6379:6379 redis:alpine

redis-stop: ## Stop Redis test container
	docker stop redis-jwt-test || true
	docker rm redis-jwt-test || true
...
docker-run: docker-build ## Build and run Docker container
	docker run --rm -p 8080:8080 -p 2019:2019 $(BINARY_NAME):$(VERSION)
